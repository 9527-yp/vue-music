<template>
    <div class="music-table">
        <table v-if="playlist.length > 0" class="song-list-table">
            <thead>
                <tr>
                    <th class="first th-1">
                        <div class="text"></div>
            ...
            <tbody>
                <tr
                  ...
                  :class="{
                  'even' : index % 2 === 0,
                  'list-disabled' : isCopyright(item?.privilege) === 0
                   }"
                >
                    <td class="tr-index">
                        <div class="index-hd">
                    ...
                    <td>
                        <div class="text">
                            <template v-if="item?.ar" v-for="(key, i) in item.ar" :key="i">
                                <span class="text-hov" :title="key.name" @click="toSinger(key.id)">{{key.name}}</span><i v-if="i !== item.ar.length-1">/</i>
                            </template>
                            <template v-if="item?.artists" v-for="(key, i) in item.artists" :key="i">
                    ...
                    <td>
                        <div class="text del">
                            <span v-if="item?.al" class="text-hov" @click="toAlbum(item?.al?.id)">{{item?.al?.name}}</span>
                            <span v-if="item?.album" class="text-hov" @click="toAlbum(item?.album?.id)">{{item?.album?.name}}</span>
                        </div>
                        <i v-if="isLogin" class="del-icn" @click="delsong(item?.id, index)"></i>
                    </td>
                </tr>
            </tbody>
        </table>
...
function addMusic(item: songType) {
    let index = isCopyright(item?.privilege)
